import type { ColorFormatValue } from './format'
...
export type ColorHarmonyAnyCase = ColorHarmony | keyof typeof Harmony

export function complementaryColor<T extends ColorFormat>(color: ColorFormatValue<T>, format: T): ColorFormatValue<T> {
  const hsl = convert[ensureColorFormat(format)].hsl(color as any)
  const newHue = hueShift(hsl.h, 180)
  return convert.hsl[ensureColorFormat(format)]({ ...hsl, h: newHue }) as ColorFormatValue<T>
}

export function analogousColors<T extends ColorFormat>(
  color: ColorFormatValue<T>,
  format: T,
  degree: number = 30,
  count: number = 1,
): ColorFormatValue<T>[] {
  ...
  return hslColors.map((hsl, index) => {
    ...
    return convert.hsl[ensureColorFormat(format)]({ ...hsl, h: newHue }) as ColorFormatValue<T>
  }) as ColorFormatValue<T>[]
}

export function triadicColors<T extends ColorFormat>(color: ColorFormatValue<T>, format: T): ColorFormatValue<T>[] {
...
export function monochromaticColors<T extends ColorFormat>(
  ...
  return shifts.map((shift) => {
    ...
    return convert.hsl[ensureColorFormat(format)]({ ...hsl, l: newLightness }) as ColorFormatValue<T>
  })
}
